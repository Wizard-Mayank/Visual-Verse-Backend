...
const createToken = (userDoc) =>
  jwt.sign({ sub: userDoc._id.toString(), email: userDoc.email }, JWT_SECRET, {
    expiresIn: "7d",
  });
...
const makeImageUrl = (req, filename) =>
  `${req.protocol}://${req.get("host")}/api/image/${filename}`;
...
const updatePasswordHandler = async (req, res) => {
  ...
  if (!email || !username || !newPassword) {
    return res
      .status(400)
      .json({ msg: "email, username and newPassword are required" });
  }
  try {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ msg: "Login successful", token, user: toClientUser(user) });
  } catch (err) {
    console.error(err.message);
    return res.status(500).json({ msg: "Server error" });
  }
...
router.post("/albums", authRequired, async (req, res) => {
  try {
    ...
    if (!albumName || !uid)
      return res.status(400).json({ msg: "albumName and uid are required" });
    if (req.auth.sub !== uid) return res.status(403).json({ msg: "Forbidden" });
    const album = await Album.create({ name: albumName, userId: uid });
    return res
      .status(201)
      .json({ id: album._id.toString(), data: { name: album.name } });
  } catch (error) {
    if (error.code === 11000)
...
router.delete("/albums/:albumId", authRequired, async (req, res) => {
  try {
    ...
    if (album.userId.toString() !== req.auth.sub)
    ...
    await Promise.all(
      photos.map((photo) => deleteFromGridFs(db, bucket, photo.filename)),
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (files.length > 3)
        return res.status(400).json({ msg: "Only 3 images can be uploaded" });
      ...
      return res
        .status(201)
        .json({ msg: "Images uploaded successfully", photos: uploaded });
    } catch (error) {
      return res.status(500).json({ msg: "Server error" });
    }
  },
...
router.delete("/photos/:id", authRequired, async (req, res) => {
  try {
    ...
    if (photo.userId.toString() !== req.auth.sub)
      return res.status(403).json({ msg: "Forbidden" });
...
router.get("/photos/root", authRequired, async (req, res) => {
  try {
    ...
    if (!uid) return res.status(400).json({ msg: "uid is required" });
    if (req.auth.sub !== uid) return res.status(403).json({ msg: "Forbidden" });
    const photos = await Photo.find({ userId: uid, albumId: "ROOT" }).sort({
      createdAt: -1,
    });
...
router.get("/photos/album", authRequired, async (req, res) => {
  try {
    ...
    const photos = await Photo.find({ userId: req.auth.sub, albumId }).sort({
      createdAt: -1,
    });
    const payload = photos.map((p) => ({
      id: p._id.toString(),
